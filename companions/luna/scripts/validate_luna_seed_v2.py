#!/usr/bin/env python3
"""Validate luna_seed_v2.jsonl against Luna v2 corpus spec (voice + schema + acceptance checks)."""
...
def corpus_checks(examples: list[dict], grounding_ids: set[str]) -> list[str]:
    ...
    for e in examples:
        ...
        if m >= 0.95:
            high_mood += 1
        if (e.get("pet") or {}).get("proactive_luna") is True:
            proactive += 1
        elif (e.get("text") or "") == "":
            proactive += 1
