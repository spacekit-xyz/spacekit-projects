#!/usr/bin/env bash
# Remove a Cairn app deployment from the storage node (manifest, content facts, marketplace listing).
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
WEBSITE_ENV="${CAIRN_WEBSITE_ENV:-$ROOT/../../../spacekit.xyz-website/.env.production}"

APP_ID="${1:-${CAIRN_APP_ID:-}}"
if [[ -z "$APP_ID" && -f "$WEBSITE_ENV" ]]; then
  APP_ID="$(grep -E '^VITE_CAIRN_NOTES_APP_ID=' "$WEBSITE_ENV" | head -1 | cut -d= -f2- | tr -d '"' | tr -d "'")"
fi

if [[ -z "$APP_ID" ]]; then
  echo "Usage: ./scripts/undeploy.sh <app-id-hex>" >&2
  echo "   or set CAIRN_APP_ID / VITE_CAIRN_NOTES_APP_ID in the website .env" >&2
  exit 1
fi
...
echo "→ Removing Cairn deployment"
echo "   App ID:       ${APP_ID}"
