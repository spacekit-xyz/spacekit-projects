#!/usr/bin/env bash
# Remove a Harmonia app deployment from the storage node (manifest, content facts, marketplace listing).
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
WEBSITE_ENV="${HARMONIA_WEBSITE_ENV:-$ROOT/../../../spacekit.xyz-website/.env}"

APP_ID="${1:-${HARMONIA_APP_ID:-}}"
if [[ -z "$APP_ID" && -f "$WEBSITE_ENV" ]]; then
  APP_ID="$(grep -E '^VITE_HARMONIA_APP_ID=' "$WEBSITE_ENV" | head -1 | cut -d= -f2- | tr -d '"' | tr -d "'")"
fi

if [[ -z "$APP_ID" ]]; then
  echo "Usage: ./scripts/undeploy.sh <app-id-hex>" >&2
  echo "   or set HARMONIA_APP_ID / VITE_HARMONIA_APP_ID in the website .env" >&2
  exit 1
fi
...
echo "→ Removing Harmonia deployment"
echo "   App ID:       ${APP_ID}"
