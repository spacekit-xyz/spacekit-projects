#!/usr/bin/env bash
# Upload a packaged Quay .spkg to the storage node (and optionally marketplace).
set -euo pipefail
...
PACKAGE="${QUAY_PACKAGE:-$UI/quay-${VERSION}.spkg}"
...
if [[ -n "$APP_ID" ]] && curl -fsS "${STORAGE_NODE}/facts/${APP_ID}" >/dev/null 2>&1; then
  ...
  QUAY_APP_ID="$APP_ID" SPACEKIT_STORAGE_NODE_URL="$STORAGE_NODE" "$ROOT/scripts/undeploy.sh" "$APP_ID"
  echo ""
fi
...
# Verify manifest on storage references the packaged JS (storage used to keep stale manifests).
...
echo "   VITE_QUAY_APP_ID=<app-id-from-output-above>"
echo ""
