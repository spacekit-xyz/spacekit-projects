#!/usr/bin/env bash
# Remove a Dragon Layerz deployment from the storage node (manifest, content facts, marketplace listing).
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
WEBSITE_ENV="${DRAGON_LAYERZ_WEBSITE_ENV:-$ROOT/../../../spacekit.xyz-website/.env}"

APP_ID="${1:-${DRAGON_LAYERZ_APP_ID:-}}"
if [[ -z "$APP_ID" && -f "$WEBSITE_ENV" ]]; then
  APP_ID="$(grep -E '^VITE_DRAGON_LAYERZ_APP_ID=' "$WEBSITE_ENV" | head -1 | cut -d= -f2- | tr -d '"' | tr -d "'")"
fi

if [[ -z "$APP_ID" ]]; then
  echo "Usage: ./scripts/undeploy.sh <app-id-hex>" >&2
  echo "   or set DRAGON_LAYERZ_APP_ID / VITE_DRAGON_LAYERZ_APP_ID in the website .env" >&2
  exit 1
fi
...
echo "→ Removing Dragon Layerz deployment"
echo "   App ID:       ${APP_ID}"
...
echo ""
echo "Next:"
echo "  1. Clear or update VITE_DRAGON_LAYERZ_APP_ID in spacekit.xyz-website/.env after redeploy"
