#!/usr/bin/env bash
# Remove a Quay app deployment from the storage node (manifest, content facts, marketplace listing).
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
WEBSITE_ENV="${QUAY_WEBSITE_ENV:-$ROOT/../../../spacekit.xyz-website/.env}"

APP_ID="${1:-${QUAY_APP_ID:-}}"
if [[ -z "$APP_ID" && -f "$WEBSITE_ENV" ]]; then
  APP_ID="$(grep -E '^VITE_QUAY_APP_ID=' "$WEBSITE_ENV" | head -1 | cut -d= -f2- | tr -d '"' | tr -d "'")"
fi

if [[ -z "$APP_ID" ]]; then
  echo "Usage: ./scripts/undeploy.sh <app-id-hex>" >&2
  echo "   or set QUAY_APP_ID / VITE_QUAY_APP_ID in the website .env" >&2
  exit 1
fi
...
echo ""
echo "Next:"
echo "  1. Clear VITE_QUAY_APP_ID from spacekit.xyz-website/.env (or set the new id after redeploy)"
echo "  2. Hard-refresh the website (Cmd+Shift+R)"
echo "  3. ./scripts/package.sh && ./scripts/deploy.sh"
