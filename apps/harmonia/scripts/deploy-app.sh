#!/usr/bin/env bash
# Upload a packaged Harmonia .spkg to the storage node (and optionally marketplace).
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
UI="$ROOT/ui"
WEBSITE_ENV="${HARMONIA_WEBSITE_ENV:-$ROOT/../../../spacekit.xyz-website/.env}"
VERSION="${HARMONIA_VERSION:-$(node -p "require('$UI/package.json').version")}"
PACKAGE="${HARMONIA_PACKAGE:-$UI/harmonia-${VERSION}.spkg}"
...
if [[ -n "$APP_ID" ]] && curl -fsS "${STORAGE_NODE}/facts/${APP_ID}" >/dev/null 2>&1; then
  ...
  HARMONIA_APP_ID="$APP_ID" SPACEKIT_STORAGE_NODE_URL="$STORAGE_NODE" "$ROOT/scripts/undeploy.sh" "$APP_ID"
  echo ""
fi
...
echo "   VITE_HARMONIA_APP_ID=<app-id-from-output-above>"
echo ""
